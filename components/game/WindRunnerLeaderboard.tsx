...
import { Trophy, Crown, ChevronDown, ChevronUp, Wind } from 'lucide-react'
...
export default function WindRunnerLeaderboard() {
    ...
    useEffect(() => {
        const fetchRankings = async () => {
            try {
                const res = await fetch('/api/ranking/windrunner')
                if (res.ok) {
            ...
            <div className="bg-black/60 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden shadow-lg">
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-200 tracking-wider">TOP RUNNERS</span>
                    </div>
                                ...
                                {rankings.length === 0 ? (
                                    ...
                                    rankings.map((user) => (
                                        <div key={user.userId} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/5 transition-colors group">
                                            ...
                                            <div className="text-xs font-mono font-bold text-purple-400 flex items-center gap-1">
