...
import { Trophy, Crown, ChevronDown, ChevronUp, Footprints } from 'lucide-react'
...
export default function StairsLeaderboard() {
    ...
    useEffect(() => {
        const fetchRankings = async () => {
            try {
                const res = await fetch('/api/ranking/stairs')
                if (res.ok) {
    ...
    // Only render if we have data or empty state is fine
            ...
            <div className="bg-black/60 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden shadow-lg">
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-200 tracking-wider">TOP CLIMBERS</span>
                    </div>
                                    ...
                                    rankings.map((user) => (
                                        <div key={user.userId} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/5 transition-colors group">
                                            ...
                                            <div className="text-xs font-mono font-bold text-cyan-400 flex items-center gap-1">
                                                ...
                                                <span className="text-[9px] text-slate-600">STEPS</span>
                                            </div>
                                        </div>
                                    ))
                                )}
