...
import { Trophy, Crown, ChevronDown, ChevronUp, Rocket } from 'lucide-react'
...
export default function SkyRoadsLeaderboard() {
    ...
    useEffect(() => {
        const fetchRankings = async () => {
            try {
                const res = await fetch('/api/ranking/skyroads')
                if (res.ok) {
            ...
            <div className="bg-black/60 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden shadow-lg">
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-200 tracking-wider">TOP PILOTS</span>
                    </div>
                                    ...
                                    rankings.map((user) => (
                                        <div key={user.userId} className="flex items-center justify-between p-2 rounded-lg hover:bg-white/5 transition-colors group">
                                            ...
                                            <div className="text-xs font-mono font-bold text-cyan-400 flex items-center gap-1">
                                                ...
                                                <span className="text-[9px] text-slate-600">KM</span>
                                            </div>
                                        </div>
                                    ))
                                )}
