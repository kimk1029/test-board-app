...
export default function SkyRoadsPage() {
    ...
    const [rankings, setRankings] = useState<{name: string, score: number}[]>(DUMMY_RANKING);
    ...
    useEffect(() => {
        const fetchRanking = async () => {
            try {
                const res = await fetch('/api/ranking/game?gameType=skyroads'); 
                if (res.ok) {
                ...
                // Fallback dummy
                setRankings([
                    { name: 'SpaceAce', score: 12500 },
                    { name: 'StarWalker', score: 9800 },
                    { name: 'VoidRunner', score: 7500 },
                    { name: 'CosmicDash', score: 5200 },
                    { name: 'MoonJumper', score: 3100 },
                ]);
            }
        };
        fetchRanking();
    ...
    const handleGameOver = (score: number) => {
        if (score > myBestScore) {
            setMyBestScore(Math.floor(score));
            localStorage.setItem('skyroads_highscore', Math.floor(score).toString());
        }
    };
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-4 relative overflow-hidden">
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start z-10 mt-4">
                {/* Leaderboard */}
                <div className="w-full lg:w-64 bg-[#0f172a] border border-white/10 rounded-2xl p-5 shadow-2xl backdrop-blur-sm order-2 lg:order-1">
                    <div className="flex items-center justify-between mb-4 border-b border-white/10 pb-3 cursor-pointer" onClick={() => setIsLeaderboardOpen(!isLeaderboardOpen)}>
                        <div className="flex items-center gap-2">
                            <Trophy className="w-5 h-5 text-yellow-400" />
                            <span className="font-bold text-sm text-yellow-400 tracking-wider">HALL OF FAME</span>
                        </div>
                        {isLeaderboardOpen ? <ChevronUp className="w-4 h-4 text-slate-500"/> : <ChevronDown className="w-4 h-4 text-slate-500"/>}
                    </div>
                    
                    {isLeaderboardOpen && (
                        <div className="space-y-3 animate-in slide-in-from-top-2">
                            {rankings.map((user, index) => (
                                <div key={index} className={`flex items-center justify-between text-sm p-3 rounded-xl transition-all ${user.name === 'YOU' ? 'bg-cyan-500/20 border border-cyan-500/30' : 'bg-white/5 hover:bg-white/10'}`}>
                                    <div className="flex items-center gap-3">
                                        {index === 0 ? (
                                            <Crown className="w-4 h-4 text-yellow-400" fill="currentColor" />
                                        ) : (
                                            <span className={`w-4 text-center font-bold ${index < 3 ? 'text-white' : 'text-slate-500'}`}>{index + 1}</span>
                                        )}
                                        <span className={`font-medium truncate max-w-[80px] ${user.name === 'YOU' ? 'text-cyan-400' : 'text-slate-300'}`}>{user.name}</span>
                                    </div>
                                    <span className="font-mono font-bold text-white">{(user.score || 0).toLocaleString()}</span>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    <div className="mt-6 pt-4 border-t border-white/10 text-center">
                        <div className="text-xs text-slate-500 uppercase mb-1">My Best Record</div>
                        <div className="text-2xl font-black text-cyan-400">{myBestScore.toLocaleString()}</div>
                    </div>
                </div>
             ...
             <div className="h-2 bg-gray-800 rounded-full overflow-hidden relative">
                 <div 
                    className={`h-full transition-all duration-200 ${
                        hudState.fuel < 30 ? 'bg-red-500 shadow-[0_0_10px_red]' : 'bg-gradient-to-r from-green-400 to-cyan-500'
                    }`}
