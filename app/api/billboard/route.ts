...
export async function GET(request: NextRequest) {
  try {
    ...
    // 1. 실제 전광판 이벤트 조회
    let billboardEvents: any[] = []
    try {
      billboardEvents = await prisma.billboardEvent.findMany({
        where: {
      ...
      console.error('Billboard events query error:', e)
      billboardEvents = []
    }
    ...
    if (formattedEvents.length < 5) {
        try {
            ...
            console.error('Top logs query error:', e)
        }
    }
